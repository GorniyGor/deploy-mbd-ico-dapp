...
export default function Home() {
  ...
  // based on the My Boy Dev NFT's held by the user for which they havent claimed the tokens
  const [tokensToBeClaimed, setTokensToBeClaimed] = useState(zero);
  // balanceOfCryptoDevTokens keeps track of number of My Boy Dev tokens owned by an address
  ...
  /**
   * getBalanceOfCryptoDevTokens: checks the balance of My Boy Dev Tokens's held by an address
   */
  ...
  const mintCryptoDevToken = async (amount) => {
    try {
      ...
      const tx = await tokenContract.mint(amount, {
        // value signifies the cost of one My Boy dev token which is "0.001" eth.
      ...
      await tx.wait();
      setLoading(false);
      window.alert("Sucessfully minted My Boy Dev Tokens");
  ...
  /**
   * claimCryptoDevTokens: Helps the user claim My Boy Dev Tokens
   */
  const claimCryptoDevTokens = async () => {
    try {
      ...
      await tx.wait();
      setLoading(false);
      window.alert("Sucessfully claimed My Boy Dev Tokens");
      await getBalanceOfCryptoDevTokens();
      await getTotalTokensMinted();
      await getTokensToBeClaimed();
    } catch (err) {
  ...
  return (
    <div>
      <Head>
        <title>My Boy Devs</title>
        <meta name="description" content="ICO-Dapp" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <div className={styles.main}>
        <div>
          <h1 className={styles.title}>Welcome to My Boys Devs (MBD) ICO!</h1>
          <div className={styles.description}>
            You can claim or mint MBD tokens here
          </div>
          {walletConnected ? (
            <div>
              <div className={styles.description}>
                {/* Format Ether helps us in converting a BigNumber to string */}
                You have minted {utils.formatEther(balanceOfCryptoDevTokens)} MBD Tokens
              </div>
              <div className={styles.description}>
                {/* Format Ether helps us in converting a BigNumber to string */}
